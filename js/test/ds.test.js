...
//=====================================================================================================
//test suite for simple Priority Queue
//=====================================================================================================
test("Priority Queue Functionality", function() {
	var pQ = BUNNY.DS.priorityQueue();
	
	//test for the length of the priority Queue
	equal(pQ.size(), 0, "Newly created priority queue has a size of 0.");
	
	//check if the newly created pQ is empty or not
	equal(pQ.isEmpty(), true, "Newly created priority queue is empty.");
	
	//check for the error message when the queue is empty
	equal(pQ.dequeue(), "Error: Priority Queue is Empty.", "Exception thrown when trying to dequeue from an empty queue.")
	...
	//create a dummy data
	var dummy = [];
	dummy.push({element: "first", priority: 2});
	dummy.push({element: "second", priority: 4});
	dummy.push({element: "third", priority: 1});
	dummy.push({element: "fourth", priority: 1});
	dummy.push({element: "fifth", priority: 3});
	
	//rerun the size and isEmpty test
	equal(pQ.size(), 5, "New size of the queue after adding 5 elements is 5.");
	equal(pQ.isEmpty(), false, "Priority queue is not empty, after adding 5 elements.");
	
	equal(pQ.dequeue()[0].element, "third", "Element - 'third' is removed since it has the highest priority.");
	equal(pQ.dequeue()[0].element, "fourth", "Element - 'fourth' is removed since it has the highest priority.");
	equal(pQ.dequeue()[0].element, "first", "Element - 'first' is removed since it has the highest priority.");
	
	//check size of the queue
	equal(pQ.size(), 2, "New length of the queue is 2.");
...
test("Simple Link List Functionality", function() {
	...
	deepEqual(linkList.getHead(), {
		"data" : "first",
		"next" : null,
		"prev" : null
	}, "After adding data at the end of the list, head of the list retrieved");
